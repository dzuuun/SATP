...
module.exports = {
  ...
  getTeacherSubject: (data, callBack) => {
    pool.query(
      "SELECT school_years.name AS school_year, semesters.name AS semester, CONCAT( teachers.surname, ', ', teachers.givenname ) AS teacher_name, subjects.code AS subject_code,subjects.id AS subject_id, subjects.name AS subject_name, departments.name AS department, colleges.name AS college FROM transactions INNER JOIN subjects ON transactions.subject_id = subjects.id INNER JOIN school_years ON transactions.school_year_id = school_years.id INNER JOIN semesters ON transactions.semester_id = semesters.id INNER JOIN teachers ON transactions.teacher_id = teachers.id INNER JOIN departments ON teachers.department_id = departments.id INNER JOIN colleges ON departments.college_id = colleges.id WHERE transactions.school_year_id = ? AND transactions.semester_id = ? AND transactions.teacher_id = ? GROUP BY subjects.id ",
  ...
  getTeacherInformation: (data, callBack) => {
    pool.query(
      "SELECT school_years.name AS school_year, semesters.name AS semester, CONCAT( teachers.surname, ', ', teachers.givenname ) AS teacher_name, departments.name AS department, colleges.name AS college, ROUND( AVG(CAST(trans_item.rate AS FLOAT)), 3 ) AS mean FROM trans_item INNER JOIN transactions ON trans_item.transaction_id = transactions.id INNER JOIN school_years ON transactions.school_year_id = school_years.id INNER JOIN semesters ON transactions.semester_id = semesters.id INNER JOIN teachers ON transactions.teacher_id = teachers.id INNER JOIN departments ON teachers.department_id = departments.id INNER JOIN colleges ON departments.college_id = colleges.id WHERE transactions.school_year_id = ? AND transactions.semester_id = ? AND transactions.teacher_id = ?",
      [data.school_year_id, data.semester_id, data.teacher_id],
      (error, results) => {
        if (error) {
          callBack(error);
