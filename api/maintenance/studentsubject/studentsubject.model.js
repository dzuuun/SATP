...
module.exports = {
  ...
  getExcludedSubjectsByStudent: (data, callBack) => {
    pool.query(
  ...
  // deleteStudentSubject: (data, callBack) => {
  //   pool.query(
  //     "SELECT user_info.user_id, school_years.name AS school_year, semesters.name AS semester, subjects.code AS subject_code, CONCAT( teachers.givenname, ' ', teachers.surname ) AS teacher_name FROM student_subject INNER JOIN school_years ON student_subject.school_year_id = school_years.id INNER JOIN semesters ON student_subject.semester_id = semesters.id INNER JOIN subjects ON student_subject.subject_id = subjects.id INNER JOIN teachers ON student_subject.teacher_id = teachers.id INNER JOIN rooms ON student_subject.room_id=rooms.id INNER JOIN user_info ON student_subject.student_id = user_info.user_id WHERE student_subject.student_id=? AND student_subject.school_year_id=? AND student_subject.semester_id=? AND student_subject.subject_id=?",
  //     [data.student_id, data.school_year_id, data.semester_id, data.subject_id],
  //     (error, result) => {
  //       pool.query(
  //         "UPDATE student_subject SET student_subject.is_excluded=?, student_subject.reason=? WHERE student_subject.student_id=? AND student_subject.school_year_id=? AND student_subject.semester_id=? AND student_subject.subject_id=?",
  //         [
  //           data.is_excluded,
  //           data.reason,
  //           data.student_id,
  ...
  // delete
  // search
  searchStudentSubject: (data, callBack) => {
    pool.query(
      "SELECT student_subject.student_id, student_subject.school_year_id, student_subject.semester_id, student_subject.subject_id, school_years.name AS school_year, semesters.name AS semester, subjects.code AS subject_code, subjects.name AS subject_name, CONCAT( teachers.givenname, ' ', teachers.surname ) AS teacher_name, CONCAT( user_info.givenname, ' ', user_info.surname ) AS student_name, student_subject.schedule_code, student_subject.time_start, student_subject.time_end, student_subject.day, rooms.name AS room FROM student_subject INNER JOIN school_years ON student_subject.school_year_id = school_years.id INNER JOIN semesters ON student_subject.semester_id = semesters.id INNER JOIN subjects ON student_subject.subject_id = subjects.id INNER JOIN teachers ON student_subject.teacher_id = teachers.id INNER JOIN rooms ON student_subject.room_id = rooms.id INNER JOIN user_info ON student_subject.student_id = user_info.user_id WHERE student_subject.is_excluded = 0 AND( semesters.name LIKE '%" +
        data.search +
        "%' OR subjects.name LIKE '%" +
        data.search +
        "%' OR teachers.surname LIKE '%" +
        data.search +
        "%' OR teachers.givenname LIKE '%" +
        data.search +
        "%' OR teachers.middlename LIKE '%" +
        data.search +
        "%' OR user_info.surname LIKE '%" +
        data.search +
        "%' OR user_info.givenname LIKE '%" +
        data.search +
        "%' OR user_info.middlename LIKE '%" +
        data.search +
        "%' OR student_subject.schedule_code LIKE '%" +
        data.search +
        "%' OR student_subject.time_start LIKE '%" +
        data.search +
        "%' OR student_subject.time_end LIKE '%" +
        data.search +
        "%' OR student_subject.day LIKE '%" +
        data.search +
        "%' OR rooms.name LIKE '%" +
        data.search +
        "%')",
