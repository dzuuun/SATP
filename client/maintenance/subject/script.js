...
async function editFormCall(id) {
  try {
    const response = await requestJson(`/api/subject/${id}`);
    const subject = response.data;
    rowIdToUpdate = subject.id;
    document.getElementById("editSubjectCode").value = subject.code;
    document.getElementById("editSubjectName").value = subject.name;
    document.getElementById("isSubjectActiveEdit").checked =
      subject.is_active == 1;
...
async function saveSubject(url, method, payload, modalId) {
  try {
    ...
    for (let index = 0; index < actions.length; index++) {
      ...
      try {
        const response = await requestJson(
          type === "created" ? "/api/subject/add" : "/api/subject/update",
          {
...
function downloadErrors(rows) {
  ...
  XLSX.writeFile(
    book,
    `Subject_Import_Errors_${new Date().toISOString().split("T")[0]}.xlsx`,
  );
}
