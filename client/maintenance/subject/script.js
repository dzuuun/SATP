...
formAddSubject.addEventListener("submit", async (event) => {
  ...
  const isActive = document.getElementById("isSubjectActive").checked;
  if (isActive == false) {
...
async function editFormCall(id) {
  await fetch(`${baseURL}/api/subject/` + id, {
    ...
    .then((response) => {
      ...
      if (data.is_active == 0) {
        document.getElementById("isSubjectActiveEdit").checked = false;
      } else {
        document.getElementById("isSubjectActiveEdit").checked = true;
      }
...
formEditSubject.addEventListener("submit", async (event) => {
  ...
  const isActive = document.getElementById("isSubjectActiveEdit").checked;
  if (isActive == false) {
    formData.append("is_active", "0");
  } else {
    formData.append("is_active", "1");
