...
let collegesByCode = new Map();

$(document).ready(() => {
  table = $("#table").DataTable({
    ajax: { url: "/api/department", dataSrc: "data", cache: true },
    columns: [
      { data: "department_code", title: "Department code", width: "18%" },
      { data: "name", title: "Department name" },
      { data: "college_code", title: "College", width: "16%" },
      {
        data: "is_active",
        title: "Status",
        width: "14%",
      ...
      },
      {
        ...
        className: "dt-center",
        render: (id) =>
          `<button class="table-edit-button" onclick="editFormCall(${id})" aria-label="Edit department"><svg viewBox="0 0 24 24"><path d="m14 5 5 5M4 20l3.5-.8L19 7.7a2.1 2.1 0 0 0-3-3L4.8 16.2 4 20Z"/></svg></button>`,
      },
    ],
    pageLength: 10,
...
async function editFormCall(id) {
  try {
    const response = await requestJson(`/api/department/${id}`);
    const department = response.data;
    rowIdToUpdate = department.id;
    ...
    document.getElementById("selectCollegeEdit").value = department.college_id;
    document.getElementById("isDepartmentActiveEdit").checked =
      department.is_active == 1;
    toggleModal("editModal", true);
  } catch (error) {
    setErrorMessage("Unable to load the department.");
  }
}
...
async function saveDepartment(url, method, payload, modalId) {
  try {
    ...
    setErrorMessage("Unable to save the department.");
  }
}
...
function classifyRows(rows, existing) {
  const byCode = new Map(
    existing.map((item) => [
      normalize(item.department_code || item.code),
      item,
    ]),
  );
  const seen = new Set();
  ...
  rows.forEach((raw, index) => {
    ...
    const name = String(raw.name || "")
      .trim()
      .replace(/\s+/g, " ");
    const collegeCode = String(raw.college_code || "")
      .trim()
      .toUpperCase();
    const college = collegesByCode.get(normalize(collegeCode));
    ...
    if (!code || !name || !collegeCode) {
      result.errors.push({
        ...base,
        reason: "Code, name, and college_code are required",
      });
    } else if (!college) {
      result.errors.push({
        ...base,
        reason: `College ${collegeCode} was not found`,
      });
    } else if (seen.has(key)) {
      result.errors.push({
        ...base,
        reason: "Duplicate department code in file",
      });
    } else {
      seen.add(key);
...
function renderPreview() {
  ...
  groups.forEach(([key, listId, countId, detail]) => {
    ...
    items.forEach((item) => {
      ...
      row.children[1].textContent = `${item.code} — ${item.name} (${item.collegeCode || "No college"})`;
    ...
    for (let index = 0; index < actions.length; index++) {
      ...
      try {
        const response = await requestJson(
          type === "created" ? "/api/department/add" : "/api/department/update",
          {
...
function downloadErrors(rows) {
  ...
  XLSX.writeFile(
    book,
    `Department_Import_Errors_${new Date().toISOString().split("T")[0]}.xlsx`,
  );
}
...
async function loadColleges() {
  try {
    ...
    ["selectCollege", "selectCollegeEdit"].forEach((selectId) => {
      const select = document.getElementById(selectId);
      colleges.forEach((college) => {
        const option = document.createElement("option");
        option.value = college.id;
        option.textContent = `${college.code || college.college_code} — ${college.name}`;
