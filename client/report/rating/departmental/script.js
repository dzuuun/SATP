...
async function loadReport() {
  ...
  try {
    const [ratingResponse, commentResponse] = await Promise.all([
      requestJson("/api/report/rating/departmental", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(query),
      }),
      requestJson("/api/report/rating/comment/departmental", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(query),
      }),
    ...
    showToast(
      error.message || "Unable to prepare the department rating report.",
    );
    showEmptyState();
  } finally {
...
function renderReport() {
  ...
  document.getElementById("department").textContent = first.department || "—";
...
async function downloadPdf() {
  ...
  try {
    ...
    pdf.setFontSize(10);
    pdf.text("DEPARTMENTAL RATING REPORT", pageWidth / 2, 50, {
      align: "center",
    });
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(8);
    const departmentLines = pdf.splitTextToSize(
      `Department: ${document.getElementById("department").textContent}`,
      pageWidth - 36,
    );
    pdf.text(departmentLines, 18, 58);
    const firstY = 59 + departmentLines.length * 4;
    const collegeLines = pdf.splitTextToSize(
      `College: ${document.getElementById("college").textContent}`,
      pageWidth - 36,
    );
    pdf.text(collegeLines, 18, firstY);
    const detailY = firstY + collegeLines.length * 4 + 1;
    const secondY = detailY + 5;
    const summaryY = secondY + 5;
    pdf.text(
    ...
    pdf.setFont("helvetica", "bold");
    pdf.text(
      `Department Average: ${document.getElementById("overallMean").textContent}`,
      pageWidth - 18,
      summaryY,
      { align: "right" },
    );
    ...
    pdf.autoTable({
      ...
      didDrawPage: () => {
        ...
        pdf.text("SATP Department Rating Report", 18, height - 7);
        pdf.text(
    ...
    pdf.save(
      `SATP Departmental Rating Report - ${departmentName} - ${new Date().toISOString().slice(0, 10)}.pdf`,
    );
  } catch (error) {
