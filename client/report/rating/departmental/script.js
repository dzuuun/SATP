...
const getdata = async () => {
  ...
  await fetch(`/api/report/rating/departmental`, {
    method: "POST",
    headers: {
    ...
    .then((response) => {
      if (response.count === 0) {
        ...
        department.innerHTML = `${response.data[0].department}`;
        ...
        tbody.innerHTML += `<th class="text-end" scope="row" >Department Average: </th>
...
const getComment = async () => {
  ...
  await fetch(`/api/report/rating/comment/departmental`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
